...
PYTHON_REQ_USE="xml(+)"
inherit gnome.org meson python-single-r1 xdg toolchain-funcs
...
src_configure() {
	...
	if tc-is-cross-compiler ; then
		export CC="$(tc-getCC) -L${ROOT}/lib -L${ROOT}/usr/lib"
	fi
	meson_src_configure
}
...
src_install() {
	...
	mv "${ED}"/usr/bin/g-ir-compiler "${ED}"/usr/bin/g-ir-compiler.wrapped
	mv "${ED}"/usr/bin/g-ir-scanner "${ED}"/usr/bin/g-ir-scanner.wrapped
	dobin "${FILESDIR}"/g-ir-compiler
	dobin "${FILESDIR}"/g-ir-scanner
	dobin "${FILESDIR}"/g-ir-scanner-qemuwrapper
	dobin "${FILESDIR}"/g-ir-scanner-lddwrapper
