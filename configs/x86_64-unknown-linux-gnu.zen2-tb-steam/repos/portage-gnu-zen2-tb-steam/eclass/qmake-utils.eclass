...
eqmake6() {
	...
	if tc-is-cross-compiler ; then
		local CBUILD_LIBC="$(echo ${CBUILD} | cut -d '-' -f 4)"
		local CHOST_LIBC="$(echo ${CHOST} | cut -d '-' -f 4)"
		([ "${CBUILD_LIBC}" = "musl" ] && [ "${CHOST_LIBC}" = "gnu" ]) && local CBUILD_LIBDIR="lib" || local CBUILD_LIBDIR=$(get_libdir)
		/usr/"${CBUILD_LIBDIR}"/qt6/bin/qmake -makefile "${args[@]}" "$@"
	else
		# NB: we're passing literal quotes in but qmake doesn't seem to mind
