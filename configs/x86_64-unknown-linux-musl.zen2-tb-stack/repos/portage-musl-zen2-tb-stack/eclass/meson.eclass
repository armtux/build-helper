...
setup_meson_src_configure() {
	...
	if tc-is-cross-compiler; then
		...
		if [ -e "${FILESDIR}/qemuwrapper" ] ; then
			cp ${FILESDIR}/qemuwrapper ${WORKDIR}/qemuwrapper
			chmod +x ${WORKDIR}/qemuwrapper
			sed -i -E "s#\[binaries]#[binaries]\nexe_wrapper = '${WORKDIR}/qemuwrapper'#" ${T}/meson.${CHOST}.${ABI}.ini || die
		fi
	fi
